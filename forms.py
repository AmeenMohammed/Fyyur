...
#genres validation method
def genreValidation(genres):
    def _validate_genre(form, field):
        error = False
        ...
        if error:
            raise ValidationError('Not an option!')

    return _validate_genre

#genres options enums
class Genres(Enum):
...
class VenueForm(Form):
    ...
    phone = StringField(
        ...
        validators=[Regexp(
            r'^[0-9\-\+]+$', 0,
            message='Pls Enter a valid phone number')]
    )
    ...
    genres = SelectMultipleField(
        'genres', validators=[
            DataRequired(),
            genreValidation([element.value for element in Genres])
            ],
        choices= Genres.elements()
    )
